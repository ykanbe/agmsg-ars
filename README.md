...
- ChatGPT.app同梱のCodex CLIを直接起動し、古いCodex.app前提のパスを避ける
- `Codex` を `gpt-5.6-luna / max` で起動する。難しい判断はAGMSGメンバーを
  増やさず、通常Codexからnative Solへ選択的に委任する
...
- `local-llm-cli` 型の対話CLIをチーム名付きで起動し、任意の
