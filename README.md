...
- Codex paneはPATH上のmonitor shimを通さず、ChatGPT.app同梱CLIを
  AGMSG内のPTYで直接起動する
- Codex paneのモデルを固定せず、Codexの設定とAGMSGのtype別spawn optionsに
  従って起動する
...
- macOS
- Git
- Codex paneを使う場合は `/Applications/ChatGPT.app`
...
```bash
scripts/update_agmsg_dev.sh --fetch --base app-v0.1.5
```

`app-v0.1.5` は現在のパッチ列で動作確認済みの公式baseです。
...
```bash
scripts/update_agmsg_dev.sh --fetch --base app-v0.1.5 --install
```
